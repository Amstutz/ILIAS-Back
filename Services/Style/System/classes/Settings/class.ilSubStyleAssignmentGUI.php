...
class ilSubStyleAssignmentGUI
{
     ...
     * Constructor
     */
    function __construct(ilSystemStyleSettingsGUI $parent_gui)
    {
        global $DIC;
    ...
    function assignStyle(ilSkinXML $skin,ilSkinStyleXML $substyle)
    {
        ...
        $tab = new ilSysStyleCatAssignmentTableGUI($this->getParentGui(), "assignStyleToCat",$skin->getId(), $style->getId(),
            $substyle->getId());
    ...
    function saveAssignment(ilSkinXML $skin,ilSkinStyleXML $substyle)
    {
        $style = $skin->getStyle($substyle->getSubstyleOf());
        try{
            ilSystemStyleSettings::writeSystemStyleCategoryAssignment($skin->getId(), $style->getId(),
                $substyle->getId(), $_GET["root_id"]);
